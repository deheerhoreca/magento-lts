...
class Html {
  ...
  public static function minifyHtml(string $html, bool $isFragment = true): string {
    ...
    if($htmlMin === null) {
      ...
      // Danger: seems like a simple string replacement, but it will break HTML canonical links and similar which cannot be relative:
      $htmlMin->doMakeSameDomainsLinksRelative([]);                   // make some links relative, by removing the domain from attributes
      
      // Danger: Suspected cause of breaking some JS or mangled HTML:
