...
class Amasty_Shopby_Model_Source_Attribute extends Amasty_Shopby_Model_Source_Abstract
{
    ...
    public function toOptionArray()
    {
        ...
        if ($hlp->isModuleEnabled('Mage_ConfigurableSwatches')) {
            $result[] = array(
                'value' => self::DT_MAGENTO_SWATCHES,
                'label' => $hlp->__('Magento Swatches (or fallback to "Labels Only")')
            );
        }

        if ($hlp->isModuleEnabled('Amasty_Conf')) {
            $result[] = array('value' => self::DT_COLOR_SWATCHES_PRO, 'label' => $hlp->__('Color Swatches Pro'));
        }
