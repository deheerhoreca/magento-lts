...
class Amasty_Shopby_Model_Catalog_Layer_Filter_Category extends Amasty_Shopby_Model_Catalog_Layer_Filter_Category_Pure
{
    ...
    protected static $appliedState = FALSE;
    ...
    public function __construct()
    {
        ...
        $this->displayType = Mage::getStoreConfig('amshopby/category_filter/display_mode');
    ...
    {
        $items = self::DT_ADVANCED == $this->displayType ? $this->getAdvancedCollection() : $this->getItems();
        return count($items);
    }
    ...
    protected function _getItemsData()
    {
        ...
        if ($items === null) {
            ...
            if ($this->displayType == self::DT_WSUBCAT) {
    ...
    {
        if (self::DT_STATIC2LVL == $this->displayType) {
            $rootCategoryId = $this->getLayer()->getCurrentStore()->getRootCategoryId();
            $result = Mage::getModel('catalog/category')->load($rootCategoryId);
        } else {
    ...
    protected function isExcluded($id)
    {
        if (is_null($this->excludedIds)) {
            $exclude = Mage::getStoreConfig('amshopby/category_filter/exclude_cat');
            if ($exclude) {
        ...
        if (is_null($this->includedIds)) {
            $include = Mage::getStoreConfig('amshopby/category_filter/include_cat');
            if ($include) {
