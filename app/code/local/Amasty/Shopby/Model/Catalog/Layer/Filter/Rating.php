...
class Amasty_Shopby_Model_Catalog_Layer_Filter_Rating extends Mage_Catalog_Model_Layer_Filter_Abstract
{
    ...
    {
        return Mage::getStoreConfig('amshopby/rating_filter/label') ?: Mage::helper('amshopby')->__('Rating Filter');
    }
    ...
    protected function _getItemsData()
    {
        ...
        if (is_null($data)) {
			...
			$data[] = array(
				'label' => $this->getLabelHtml(self::NOT_RATED_STARS),
				'value' => ($currentValue == self::NOT_RATED_STARS) ? null : self::NOT_RATED_STARS,
				'count' => $count[5],
				'real_count' => $count[5],
				'option_id' => self::NOT_RATED_STARS,
			);
    ...
    {
        if ($countStars == self::NOT_RATED_STARS) {
            return Mage::helper('amshopby')->__(self::NOT_RATED_LABEL);
        }
        $enableWorkaroundForLestiFpc = false;
        if ($enableWorkaroundForLestiFpc) {
            $html = '<div class="ratings" style="display: inline-block;margin: 0px;padding: 0px;">
    <div class="rating-box" style="margin:0px;">
        <span class="rating" style="width:' . $countStars * 20 . '%;"></span>
    </div>
</div>';
            if ($countStars != 5) {
                $html .= '<span style="font-size: smaller">&amp; up</span>';
            }
        } else {
