...
class Amasty_Shopby_Block_Adminhtml_Value_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function prepareFieldsetFeatured()
    {
        ...
        $fldSet->addType('multistoreinput','Amasty_Shopby_Lib_Varien_Data_Form_Element_Multistoreinput');
        $fldSet->addField('url_alias', 'multistoreinput', array(
            'label'     => $this->__('URL Alias'),
            'name'      => 'url_alias',
            'required'  => false,
            'note'      => 'Write preferred SEO-friendly URL key to be added to category URL when filtered by this value.<br><b>Attention! Old URLs will stop working. New URLs based on custom URL Alias changes will work only.</b>',
        ));
    ...
    protected function prepareFieldsetMain()
    {
        ...
        $note = new Varien_Data_Form_Element_Note(array(
            'text' => '<p style="color:orangered">' . $this->__('You need to enable option <b>%s</b> on filter edit page in order to make these settings work on non-brand pages.', $this->__('Show on List')) . '</p>',
        ));
        ...
        $fldMain->addType('multistoreselect','Amasty_Shopby_Lib_Varien_Data_Form_Element_Multistoreselect');
        $fldMain->addField('cms_block_id', 'multistoreselect', array(
            'label'     => $this->__('Top CMS Block'),
            'name'      => 'cms_block_id',
            'values'    => $cmsBlocks,
        ));
        $fldMain->addField('cms_block_bottom_id', 'multistoreselect', array(
    ...
    protected function prepareFieldsetProductView()
    {
        ...
        $note = new Varien_Data_Form_Element_Note(array(
            'text' => $this->__(
                'This image will be displayed near the products having current attribute option selected. Please see <a href="%s" target="_blank">the User Guide</a> for more information.',
                Mage::helper('amshopby')->getGuideLink('filter_options_meta_title_and_description')
            ),
        ));
