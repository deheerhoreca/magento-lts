...
class Amasty_Shopby_Block_Adminhtml_Filter_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareFieldsetGeneral()
    {
        ...
        if ($this->isDecimal()) {
            ...
            $fldSet->addField(
                'number_options_for_show_search',
                'text',
                array(
            ...
            $fldSet->addField(
                ...
                array(
                    ...
                    'name' => 'max_options',
                    'note' => $this->__(
                        'Applicable for `Labels Only`, `Images only` and `Labels and Images` 
                        display types. Zero means all options are unfolded'
                    )
                )
            );
        ...
        $fldSet->addField(
            ...
            array(
                ...
                'name' => 'collapsed',
                'values' => $this->yesno,
                'note' => $this->__('Will be collapsed until customer selects any filter option')
            )
        );
    ...
    protected function _prepareFieldsetBlocks()
    {
        ...
        $fldSet2->addField('show_on_view', 'select', array(
            ...
            'values'    => $this->yesno,
            'note'      => $this->__(
                'Show options images block on product view page. Please see <a href="%s" target="_blank">the User Guide</a> for more information.',
    ...
    protected function _prepareFieldsetSpecial()
    {
        ...
        $fldSet->addField('depend_on_attribute', 'text', array(
            'label'     => $this->__('Show only when any options of the following attributes are selected'),
    ...
    protected function _toHtml()
    {
        return Mage::helper('amshopby')
                ->getGuideHint('layered_navigation_filters')
            . parent::_toHtml();
    }
}
