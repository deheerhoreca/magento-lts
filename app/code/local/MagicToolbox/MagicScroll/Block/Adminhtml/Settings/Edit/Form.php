...
class MagicToolbox_MagicScroll_Block_Adminhtml_Settings_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
...
<script type="text/javascript">
    ...
    var fieldsets = getElementsByClass("magicscroll-fieldset");
    var header = null;
    var buttons = null;
    var magicscrollFieldsetId = "";
    for (var i = 0, l = fieldsets.length; i < l; i++) {
        ...
        if (magicscrollFieldsetId != id) {
            magicscrollFieldsetId = id;
        } else {
            header.click();
        }
    }
    ...
    for (var i = 0; i < magictoolboxOptionsLength; i++) {
        ...
        var idMatch = optionName.match(/^magicscroll\[([^\[]+)\]\[([^\[]+)\]\[([^\[]+)\]/i);
            ...
            if (idMatch[2] == "default") {
                ...
                magictoolboxOptions[i].onchange = function() {
                    ...
                    for (var profile in magictoolboxPofiles) {
                        var elements = document.getElementsByName("magicscroll["+thisPlatform+"]["+profile+"]["+thisId+"]");
                        if (elements.length) {
                            if ((elements[0].tagName.toLowerCase() == "select" || elements[0].getAttribute("type") == "text") &&
                                ...
                                var radios = document.getElementsByName("magicscroll["+thisPlatform+"][default]["+thisId+"]");
                                var defaultValue = "";
                                var j;
