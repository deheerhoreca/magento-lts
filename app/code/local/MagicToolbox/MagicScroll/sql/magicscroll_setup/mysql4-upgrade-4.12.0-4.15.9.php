...
if (!empty($oldModulesInstalled)) {
    ...
    $mtResult = $connectionObject->query("SELECT * FROM `{$this->getTable('magicscroll/settings')}`");
    if ($mtResult) {
        while ($mtRow = $mtResult->fetch(PDO::FETCH_ASSOC)) {
            if (!empty($mtRow['value'])) {
                ...
                $installer->run("UPDATE `{$this->getTable('magicscroll/settings')}` SET `value` = '{$mtSettings}' WHERE `setting_id` = {$mtRow['setting_id']}");
            }
        }
    }
}
