...
class Afterpay_Afterpay_Model_Portfolios_Abstract extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function isAvailable($quote = null)
    {
        ...
        /*
        if($portfolioIpCheckEnabled) {
            $allowedIpsCloud = Mage::helper('afterpay')->getAllowedIps();
            if(is_array($allowedIps)) {
                $allowedIps = array_merge($allowedIps, $allowedIpsCloud);
            }
        }
        */
        ...
        // Checks if the current quote's base grandtotal is above the minimum amount allowed for this portfolio
        if (!empty($minAmountAllowed)
            && ( $quote->getBaseGrandTotal() < $minAmountAllowed )
            ) {
            return false;
        }

        // Checks if the current quote's base grandtotal is under the maximum allowed for this portfolio
        if (!empty($maxAmountAllowed)
            && ( $quote->getBaseGrandTotal() > $maxAmountAllowed )
            ) {
            return false;
        }
    ...
    protected function _assignData($data)
    {
        ...
        if (Mage::getStoreConfig('afterpay/afterpay_' . $this->_code . '/portfolio_type', Mage::app()->getStore()->getId()) == 'B2B') {
            ...
            $b2CInfoArray = array(
                ...
                'installmentprofile' => $data->getInstallmentprofile(),
