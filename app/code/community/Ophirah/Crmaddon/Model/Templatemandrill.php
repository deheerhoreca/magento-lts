...
/**
 * Class Ophirah_Crmaddon_Model_Core_Email_Templatemandrill
 */
class Ophirah_Crmaddon_Model_Core_Email_Templatemandrill extends Ebizmarts_Mandrill_Model_Email_Template
{
    ...
    public function send($email, $name = null, array $variables = [])
    {
        ...
        $email = ['subject' => $this->getProcessedTemplateSubject($variables), 'to' => []];
        $mail = $this->getMail();
        ...
        if (isset($variables['tags']) && count($variables['tags'])) {
            ...
            if (isset($templates[$templateId]) && isset($templates[$templateId]['label'])) {
                $email ['tags'] = [substr($templates[$templateId]['label'], 0, 50)];
            } else {
                if ($this->getTemplateCode()) {
                    $email ['tags'] = [substr($this->getTemplateCode(), 0, 50)];
                } else {
                    if ($templateId) {
                        $email ['tags'] = [substr($templateId, 0, 50)];
                    } else {
                        $email['tags'] = ['default_tag'];
                    }
                }
            }
        }
    ...
    public function getMail()
    {
        ...
        if ($this->_mail) {
            ...
            $this->_mail = new Ophirah_Crmaddon_Model_Core_Email_Mandrillmessage(Mage::getStoreConfig(Ebizmarts_Mandrill_Model_System_Config::APIKEY, $storeId));
            return $this->_mail;
        }
    }
}
