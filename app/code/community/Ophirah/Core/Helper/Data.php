...
class Ophirah_Core_Helper_Data extends Mage_Core_Helper_Abstract
{
     ...
     * That guide is added for users that extracts the archive instead of installing it,
     * so removing it is the best thing to do, otherwise users end up with a pdf file in the root.
     */
    public function removeQuickGuide()
    {
        $quickGuidePath = Mage::getBaseDir() . DS . $this::QUICK_GUIDE_PATH;

        //only remove when the file exists
        if (file_exists($quickGuidePath)) {
            unlink($quickGuidePath);
        }
    }
}
