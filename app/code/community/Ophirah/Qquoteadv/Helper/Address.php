...
class Ophirah_Qquoteadv_Helper_Address extends Mage_Core_Helper_Abstract
{
    ...
    {
        return [self::ADDRESS_TYPE_BILLING, self::ADDRESS_TYPE_SHIPPING];
    }
    ...
    public function addAddress($quoteId, $addressData, $check = null)
    {
        ...
        foreach ($addressTypes as $type) {
            ...
            if (isset($addData) && ($prevData == $addData) && ($addData != null)) {
                $sameAsBillling = '1';
            }
     ...
     *
     * @param int $quoteId
     * @return bool / Ophirah_Qquoteadv_Model_Mysql4_Quoteaddress_Collection
     */
     ...
     *  is selected
     *
     * @param array $paramsAddress // Addres Params from post
    ...
    public function buildAddress($paramsAddress)
    {
        ...
        if ($emptyBillField === true || $regionIsSet === false) {
            $paramsAddress['billing'] = [];
        }
        if ($emptyShipField === true || $regionIsSet === false) {
            $paramsAddress['shipping'] = [];
        }
     ...
     * @param int $quoteId
     * @param string $type
     * @return bool | Ophirah_Qquoteadv_Model_Quoteaddress
     */
                ...
                if ($address instanceof Mage_Customer_Model_Address) {
                    foreach ($requiredFormFields as $requiredField) {
                        if (is_array($requiredField)) {
                            ...
                            if (!$found && isset($requiredFieldOption)) {
                                $success = false;
