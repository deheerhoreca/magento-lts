...
final class Ophirah_Qquoteadv_Helper_Data extends Mage_Core_Helper_Abstract
{
                        ...
                        if (is_array($v)) {
                            ...
                            foreach ($v as $sk => $sv) {
                                try {
                                    ...
                                    } elseif ($product->getOptionById($k)->getType() == "date_time") {
                                    ...
                                    } elseif ($product->getOptionById($k)->getType() == "file") {
                                        $value = $v['title'];
                                    } else {
                                    ...
                                    if ($product->getOptionById($k)->getType() == "image" || $product->getOptionById($k)->getType() == "imagecheckbox") {
                                        ...
                                        if ($collection->getSize() > 0) {
                                            ...
                                            if ($showTitle == '1') {
                                                $finalValue .= '<img src="' . $url . '" alt="' . $title . '"/><br/><span class="cart-image-option-title">' . $title . '</span>';
                                            } else {
                                                $finalValue .= '<img src="' . $url . '" alt="' . $title . '"/><br/>';
                                            }
                                        }
                                    } else {
                            ...
                            try {
                                ...
                                if ($product->getOptionById($k)->getType() == "image" || $product->getOptionById($k)->getType() == "imagecheckbox") {
                                    ...
                                    if ($collection->getSize() > 0) {
                                        ...
                                        if ($showTitle == '1') {
                                            $value = '<img src="' . $url . '" alt="' . $title . '"/><br/><span class="cart-image-option-title">' . $title . '</span>';
                                        } else {
                                            $value = '<img src="' . $url . '" alt="' . $title . '"/>';
                                        }
    ...
    public function getConfigurableFinalPrice($productId, $selectedAttributes)
    {
        ...
        if (isset($optionPrices) && !empty($optionPrices)) {
    ...
    public function getVirtualQuoteObject($quote, $storeId, $reUseObject = false)
    {
        ...
        if (count($virtualQuote->getAddressesCollection()) > 0) {
            ...
            $data = ['country_id' => $shippingAddress->getData('country_id'),
        ...
        if (!$this->hasTaxAddressDetails($virtualQuote)) {
            ...
            $data = ['country_id' => $country, 'region_id' => $region];
            $virtualQuote->getBillingAddress()->addData($data);
            $virtualQuote->getShippingAddress()->addData($data);
        }
    ...
    public function addItemToVirtualQuote(
        ...
        try {
            //clean this cache on clone to make sure this data is set
    ...
    public function getQuoteItem(
        ...
        try {
            ...
            if (!isset($virtualQuote) || $virtualQuote == null) {
                return null;
            }
    ...
    public function getAllowedAdmins($storeId = null, $currentAdminId = null, $customerId = null)
    {
        ...
        if(Mage::getStoreConfig('qquoteadv_sales_representatives/quote_assignment/limit_available_salesrep', $storeId)){
            ...
            return $collection = Mage::getModel('admin/user')
                ->getCollection()
                ->addFieldToFilter('user_id', ['in' => $adminIds]);
        } else {
    ...
    public function isQuoteable($product, $qty)
    {
        ...
        if (!$resultQuantities->getHasError() && (!isset($resultIncrement) || !$resultIncrement->getHasError())) {
...
//                            }
    ...
    public function getAdminUser()
    {
        ...
        /** @var Mage_Core_Model_Session $currentSession */
    ...
    public function getNextAssignToAdmin($roleId = null)
    {
        ...
        /** @var Ophirah_Qquoteadv_Model_Qqadvrotation $model */
    ...
    public function getPreviousAssignedAdmin(
        ...
        if ($quote !== null) {
            ...
            $collection->addFieldToFilter('main_table.quote_id', [$filter => $quoteId]);
        }
        ...
        if ($backend === false) {
            ...
            $collection->getSelect()
                ->joinInner(
                    ['u' => $collection->getResource()->getTable('admin/user')],
     ...
     * @return bool|string (array of articles in JSON)
     */
    public function executeArticleRequest()
    {
        try {
            ...
            $articleArray['articles'] = []; // The array of articles
