...
class Ophirah_Qquoteadv_Model_Pdf_Qquote extends Mage_Sales_Model_Order_Pdf_Abstract
{
    ...
    protected function renderPlaceholderImage()
    {
        ...
        if (!isset($imagePath) || empty($imagePath)) {
            $baseDir = Mage::getDesign()->getSkinBaseDir(['_area' => 'frontend']);
    ...
    protected function getImageType($imagePath)
    {
        if (is_file($imagePath)) {
            if(function_exists('exif_imagetype')) {
                ...
                $extensions = ["GIF", "JPEG", "PNG", "SWF", "PSD", "BMP", "TIFF_II", "TIFF_MM", "ICO"];
    ...
    public function getPdf($quotes = [])
    {
        ...
        if ($quotes instanceof Ophirah_Qquoteadv_Model_Qqadvcustomer) {
            ...
            Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesafe_final', ['quote' => $quoteadv]);
            $quoteadv->save();
            Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $quoteadv]);
        ...
        $this->columns = [
            [Mage::helper('catalog')->__('Product image'), $this->_leftTextPad, $this->y, 'UTF-8'],
            [Mage::helper('catalog')->__('Product name'),  $this->leftMarginProductName, $this->y, 'UTF-8'],
            [Mage::helper('catalog')->__('SKU'),           $this->leftMarginSku, $this->y, 'UTF-8'],
            [Mage::helper('catalog')->__('QTY'),           $this->leftMarginQty, $this->y, 'UTF-8'],
            [Mage::helper('catalog')->__('Price'),         $this->leftMarginPrice, $this->y, 'UTF-8'],
            [Mage::helper('adminhtml')->__('Subtotal'),    $this->leftMarginSubtotal, $this->y, 'UTF-8']
        ];
    ...
    public function draw($unit, $page)
    {
        ...
        $requestedProductData->getSelect()->order(['product_id ASC', 'request_qty ASC']);
        ...
        $txt1[] = ['text' => $productQty, 'font' => 'regular', 'font_size' => $size];
        $txt2[] = $price; //480
        $txt3[] = $rowTotal; //542
        ...
        if (count($requestedProductData) > 1):
            foreach ($requestedProductData as $product) {
                ...
                if ($showTier === true) {
                    ...
                    $size = $this->fontSizeSmall;
                    $font = 'italic';
                    $txt1[] = ['text' => $productQty, 'font' => $font, 'font_size' => $size]; //405
                    $txt2[] = ['text' => $price, 'font' => $font, 'font_size' => $size]; //480
                    $txt3[] = ['text' => $rowTotal, 'font' => $font, 'font_size' => $size]; //542
                }
                $k++;
            }
        endif;
        ...
        $page = $this->drawLineBlocks($page, $drawItems, ['table_header' => true]);
...
//        $proposalDate = $this->_quoteadv->getProposalSent();  //Mage::helper('core')->formatDate( date( 'D M j Y')
    ...
    public function insertShippingMethods()
    {
        ...
        $boxHeader = 15;
        $boxMargin = ['top' => 10, 'right' => 0, 'bottom' => 10, 'left' => 0];
        $boxPadding = ['top' => 10, 'right' => 5, 'bottom' => 10, 'left' => 0];
    ...
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = [])
    {
        foreach ($draw as $itemsProp) {
            ...
            if (isset($column['min-height'])) {
                if (isset($maxHeight) && ($maxHeight < $column['min-height'])) {
