...
class Ophirah_Qquoteadv_Model_Observer
{
    ...
    public function updateStatusRequest()
    {
        ...
        $items->addFieldToFilter('is_quote',  ['eq' => 1]);
    ...
    public function updateStatusProposal()
    {
        ...
        $collection->addFieldToFilter('status', ['in' => $quote_status]);
    ...
    public function quoteCancelation($observer)
    {
        ...
        if ($product && $product->getId()) {
            ...
            $_collection->getSelect()->join(['p' => $table], 'main_table.quote_id=p.quote_id', []);
            $_collection->addFieldToFilter("status", ["neq" => Ophirah_Qquoteadv_Model_Status::STATUS_CANCELED]);
            ...
            foreach ($quoteIds as $quoteId) {
                ...
                try {
                    Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesafe_final', ['quote' => $quote]);
                    Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforecancel', ['quote' => $quote]);
                    $quote->save();
                    Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $quote]);
                    Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftercancel', ['quote' => $quote]);
                } catch (Exception $e) {
                    Mage::log('Exception: ' . $e->getMessage(), null, 'c2q_exception.log', true);
                }
            }
        ...
        foreach ($quoteItems as $item) {
            ...
            try {
                ...
                if ($item->getData('qty') != $product->getData('qty')) {
                    ...
                    $productPrice = Mage::getModel('qquoteadv/requestitem')->getCollection()
                        ...
                        ->addFieldToFilter('request_qty', ['eq' => $item->getData('qty')]);
            ...
            if ($product->getTaxClassId() === null || $product->getTaxClassId() == 0) {
                ...
                foreach ($item->getChildren() as $child) {
                    ...
                    } elseif ($taxClass != $child->getProduct()->getTaxClassId()) {
    ...
    public function logAdminSession(){
        ...
        if($currentSession->getUser()){
            ...
            if ($logAdmin->getData("id")) {
                // update
                $data = ['admin_id' => $adminId,'session_id' => $currentSessionId];
