...
class Ophirah_Qquoteadv_Model_Api extends Mage_Api_Model_Resource_Abstract
{
    ...
    public function setimported($params)
    {
        ...
        if (isset($params['quote_id']) && isset($params['value'])) {
            ...
            try {
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesafe_final', ['quote' => $_quoteCollection]);
                $_quoteCollection->save();
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $_quoteCollection]);
            } catch (Exception $e) {
                Mage::log('Exception: ' . $e->getMessage(), null, 'c2q_exception.log', true);
                $data = ['request_data' => $params, 'exception' => $e->getMessage()];
                $exceptions[] = $data;
            }
