...
class Ophirah_Qquoteadv_Model_Qquoteadv_Api extends Mage_Api_Model_Resource_Abstract
{
    ...
    public function setimported($params)
    {
        ...
        if (isset($params['quote_id']) && isset($params['value'])) {
            ...
            try {
                ...
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesafe_final', ['quote' => $_quote]);
                $_quote->save();
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $_quote]);
            } catch (Exception $e) {
    ...
    public function add_products_to_quote($params)
    {
        ...
        if (!count($errors) && isset($_qquoteadv) && isset($productIds) && isset($quoteId)) {
            $hasOptions = false;
            $options = false;
            ...
            foreach ($productIds as $productId) {
                ...
                $attribute = serialize(['product' => $productId, 'qty' => $qty]);
    ...
    public function update_quote_status($params)
    {
        ...
        if (!count($errors) && isset($_qquoteadv)) {
            $_qquoteadv->setStatus($params['status']);

            try {
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesafe_final', ['quote' => $_qquoteadv]);
                $_qquoteadv->save();
                Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $_qquoteadv]);
                $response = [
                    'success' => true
                ];
            } catch (Exception $e) {
