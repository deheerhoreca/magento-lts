...
class Ophirah_Qquoteadv_Model_Massupdate
{
    ...
    {
        $query_action_products = [];
        $query_action_products['inserts'] = [];
        $query_action_products['updates'] = [];
    ...
    private function getStatusProductAllowedToQuote($attribute_id, $entity_id, $entity_type_id, $mode)
    {
        ...
        $select = $connection->select()->from($this->getTableName(), ['value'])->where('attribute_id=?', $attribute_id)->where('entity_id=?', $entity_id)->where('entity_type_id=?', $entity_type_id);
        $row = $connection->fetchRow($select);
        $connection->closeConnection();

        if (!$row) {
            $status = $this::ALLOW_TO_QUOTEMODE_NOT_SET;
        } elseif ($row['value'] == $mode) {
