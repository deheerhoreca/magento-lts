...
class Ophirah_Qquoteadv_Model_Extraoptions extends Mage_Sales_Model_Quote
{
    ...
    public function getShippingService()
    {
        ...
        $collection = Mage::getModel('qquoteadv/extraoptions')->getCollection()
            ->addFieldToFilter('option_type', ['eq' => $optionType])
            ->addFieldToFilter('value', ['in' => $selected])
        ...
        $vars['required'] = Mage::getStoreConfig('qquoteadv_quote_form_builder/options/require_delivery_options') == 2;
    ...
    public function getMultipleoptionData($extraOptions, $optionType)
    {
        ...
        $collection = Mage::getModel('qquoteadv/extraoptions')->getCollection()
            ->addFieldToFilter('option_type', ['eq' => $optionType])
            ->addFieldToFilter('value', ['in' => $extraOptions])
            ->addFieldToFilter('status', 1);
