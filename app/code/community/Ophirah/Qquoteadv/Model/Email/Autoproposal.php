...
class Ophirah_Qquoteadv_Model_Email_Autoproposal extends Mage_Core_Model_Abstract
{
    ...
    public function sendEmail($forceAutoProposal = false)
    {
        ...
        if ($this->isAllowed(true, $forceAutoProposal)) {
            Mage::dispatchEvent('ophirah_qquoteadv_addSendMail_before', ['template' => $this->_template]);
            ...
            Mage::dispatchEvent('ophirah_qquoteadv_addSendMail_after', ['template' => $this->_template, 'result' => $res]);
        }
    ...
    protected function prepareEmail()
    {
        $this->loadTemplate()
            ...
            ->setEmailVar('link', Mage::getUrl("qquoteadv/view/view/", ['id' => $this->getQuote()]))
            ->setDoc()
            ->setPdfAttachment()
            ->addBcc();
