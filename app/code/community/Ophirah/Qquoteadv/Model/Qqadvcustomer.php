...
class Ophirah_Qquoteadv_Model_Qqadvcustomer extends Mage_Sales_Model_Quote
{
     ...
     * Used in quote backend
     *
     * @param bool // if $short is 'true' the 'address' and 'items' objects will be left out
     * @return array
     */
    ...
    protected function _beforeSave()
    {
        ...
        parent::_beforeSave();
        Mage::dispatchEvent('qquoteadv_qqadvcustomer_beforesave', ['quote' => $this]);
    ...
    {
        Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersave', ['quote' => $this]);
        return $this;
    }
     ...
     *
     * @param int $id quote id to be updated
     ...
     * @return  bool|object      // Mage_Customer_Model_Customer
     */
     ...
     * @return  false|object // Mage_Customer_Model_Customer
     */
    ...
    public function getFullPath()
    {
        ...
        if ($valid) {
            ...
            return self::getUploadPath(['dir' => $this->getData('quote_id'), 'file' => $path]);
        }
    }
    ...
    public function sendExpireEmail()
    {
        $expiredQuotes = $this->getCollection()
            ->addFieldToFilter('status', ['in' => [50, 53, 56, 57]])
            ->addFieldToFilter('no_expiry', ['eq' => 0])
            ->addFieldToFilter('is_quote',  ['eq' => 1])
            ->addFieldToFilter('expiry', ['eq' => date('Y-m-d')]);
        ...
        foreach ($expiredQuotes as $expiredQuote) {
            ...
            if ($template != $disabledEmail){
                 ...
                 */
                $template->setData('c2qParams', ['email' => $_quoteadv->getEmail(), 'name' => $_quoteadv->getFirstname()]);
            ...
            $_quoteadv->save();
            Mage::dispatchEvent('qquoteadv_qqadvcustomer_aftersafe_final', ['quote' => $_quoteadv]);
        }
    }
    ...
    public function sendReminderEmail()
    {
        if (Mage::getStoreConfig('qquoteadv_quote_configuration/expiration_times_and_notices/send_reminder') > 0) {
            ...
            $reminderQuotes = $this->getCollection()
                ->addFieldToFilter('status', ['in' => [50, 52, 53, 56, 57]])
                ->addFieldToFilter('no_reminder', ['eq' => 0])
                ->addFieldToFilter('is_quote', ['eq' => 1])
                ->addFieldToFilter('reminder', ['eq' => date('Y-m-d')]);
    ...
    public function exportQuotes($customerCollection, $filePath)
    {
        ...
        foreach ($customerCollection as $qquote) {
            ...
            foreach ($collection as $item) {
                ...
                $productFields = [$baseProductId, $productName, $productAttributes, $qty, $SKU];
...
//    }
    ...
    public function recalculateFixedPrice($recalPrice = [])
    {
        ...
        $recalValue = null;
        $recalType = null;
        $recalPriceTypes = ['fixed' => 1, 'percentage' => 2];
     ...
     * @return array|object      //quote items in $this->_requestedItems
     */
    public function getAllRequestItems($removeTax = true, $forceReload = false, $keepOnObject = true)
    {
        if ($this->_requestItems == null || $forceReload) {
            $qqadvproductData = Mage::getModel('qquoteadv/qqadvproduct')->getCollection()->addFieldToFilter('quote_id', ["eq" => $this->getQuoteId()]);
            ...
            foreach ($qqadvproductData as $qqadvproduct) {
                ...
                $quoteItems = Mage::getModel('qquoteadv/requestitem')->getCollection()
                    ->addFieldToFilter('quote_id', ["eq" => $qqadvproduct->getQuoteId()])
                    ->addFieldToFilter('request_qty', ["eq" => $qqadvproduct->getQty()])
                    ->addFieldToFilter('quoteadv_product_id', ["eq" => $qqadvproduct->getId()])
                    ->load();
...
//                $this->save();
            ...
            if ($handle) {
                while (false !== ($entry = readdir($handle))) {
                    ...
                    if (!empty($file_parts['extension'])) {
                        $pathOfFile = Mage::getModel('qquoteadv/qqadvcustomer')->getUploadPath(['dir' => $this->getId(), 'file' => $entry]);
                        $html .= '<tr>
     ...
     *
     * @param Ophirah_Qquoteadv_Model_Mysql4_Requestitem_Collection $quoteItems
     * @return mixed
     */
