    ...
    public function addQuoteLink()
    {
        ...
        if ($parentBlock && Mage::helper('core')->isModuleOutputEnabled('Ophirah_Qquoteadv')) {
            if(Mage::getStoreConfig('qquoteadv_general/quotations/active_c2q_tmpl')){
                ...
                $parentBlock->addLink($text, 'qquoteadv/index', $text, true, [], 50, null, 'class="top-link-qquoteadv"');
            }
        }
        return $this;
    }
        ...
        if ($parentBlock && Mage::helper('core')->isModuleOutputEnabled('Ophirah_Qquoteadv')) {
            if(Mage::getStoreConfig('qquoteadv_general/quotations/active_c2q_tmpl')){
                ...
                if ($helper->getAdminUser() === null) {
                    $parentBlock->addLink('Sales representative', 'javascript:adminLogin(\'' . Mage::helper("adminhtml")->getUrl("adminhtml/index/login/") . '\');', 'Sales representative', false, [], 50, null, 'id="top-link-qquoteadv-admin"');
                }
            }
        }
        return $this;
