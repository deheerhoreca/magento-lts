...
class Ophirah_Qquoteadv_Block_Adminhtml_Qquoteadv_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
    ...
    protected function  _prepareLayout()
    {
        ...
        if ($this->getRequest()->getParam('followup')){
            ...
            $data = ['label' => Mage::helper('qquoteadv')->__('Reset Follow Up'),
                'onclick' => 'setLocation(\'' . $this->getUrl('*/*/*') . '\')',
                'class' => ''
            ];

        } else {
            $data = ['label' => Mage::helper('qquoteadv')->__('Follow Up'),
    ...
    protected function _prepareCollection()
    {
        $collection = Mage::getModel('qquoteadv/qqadvcustomer')->getCollection()
            ...
            ->addFieldToFilter('status', ['gt' => Ophirah_Qquoteadv_Model_Status::STATUS_BEGIN]);

        if (Mage::helper('qquoteadv/licensechecks')->showFreeUserOptions()) {
            $collection->walk('setFirstname', [self::FREE_USER_QUOTES]);
            $collection->walk('setLastname', [self::FREE_USER_QUOTES]);
            $collection->walk('setEmail', [self::FREE_USER_QUOTES]);
        }
        ...
        if (Mage::helper('qquoteadv/license')->isTrialVersion(null, true)) {
            ...
            // Filter Collection
            $collection = $collection->addFieldToFilter('quote_id', ['in' => $filterArray]);
        }
        ...
        if ($this->getRequest()->getParam('followup') == '1' && Mage::getStoreConfig('qquoteadv_advanced_settings/backend/followup') != 2) {
            $collection->addFieldToFilter('no_followup', '0')
                ->addFieldToFilter('followup', ['notnull' => 1]);
            $followupIds = $this->getFollowupIds(clone $collection);

            if (is_array($followupIds)) {
                // Filter Collection
                $collection = $collection->addFieldToFilter('quote_id', ['in' => $followupIds]);
            }
        ...
        if ($this->getRequest()->getParam('followup') == '1' && Mage::getStoreConfig('qquoteadv_advanced_settings/backend/followup') == 2) {
            $collection->addFieldToFilter(
                ...
                [
                    ['eq'=>Ophirah_Qquoteadv_Model_Status::STATUS_BEGIN_ACTION_OWNER],
                    ['eq'=>Ophirah_Qquoteadv_Model_Status::STATUS_PROPOSAL_ACTION_OWNER],
                    ['eq'=>Ophirah_Qquoteadv_Model_Status::STATUS_REQUEST_ACTION_OWNER],
                ]
            );
    ...
    protected function _prepareColumns()
    {
        ...
        if(Mage::getStoreConfig('qquoteadv_advanced_settings/backend/customer_name_select') != 1){
            ...
            $collection->getSelect()
                ...
                ->joinLeft(
                    ['customer_firstname' => $firstnameAttributeTable],
                ...
                ->joinLeft(
                    ['customer_middlename' => $middlenameAttributeTable],
                ...
                ->joinLeft(
                    ['customer_lastname' => $lastnameAttributeTable],
                    'main_table.customer_id = customer_lastname.entity_id and customer_lastname.attribute_id = '.$lastnameAttribute->getId(),
                    ['customer_name' => 'CONCAT_WS(" ", customer_firstname.value, customer_middlename.value, customer_lastname.value)']
                )
    ...
    protected function _prepareMassaction()
    {
        ...
        $this->getMassactionBlock()->addItem('set_followup', [
    ...
    {
        return $this->getUrl('adminhtml/qquoteadv/edit', ['id' => $row->getId()]);
    }
}
