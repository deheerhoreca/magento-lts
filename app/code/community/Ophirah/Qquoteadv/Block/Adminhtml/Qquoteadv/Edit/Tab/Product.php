...
//        $this->setTemplate('qquoteadv/product.phtml'); @deprecated: now managed in the qquoteadv.xml layout file
    ...
    {
        //0.00 is also a price
        return $this->getQuoteShipPrice() > -1;
    }
    ...
    {
        return $this->getUrl('adminhtml/customer/edit', ['id' => $customer_id]);
    }
    ...
    public function getQtyHtml(
        ...
        if (count($collection) > 0):
            ...
            foreach ($collection as $requested_item):
                ...
                if ($hideRadio) {
                    $radioStates = ['unselected' => $disabledHtmlRadio, 'selected' => 'checked="checked"'];
                ...
                $url = $this->getUrl('*/*/deleteQtyField', ['request_id' => $requested_item->getId()]);
