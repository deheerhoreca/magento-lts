...
class Ophirah_Qquoteadv_Block_Catalog_Product_View_Addtoquotebutton extends Mage_Catalog_Block_Product_View
{
    ...
    {
        $isEnabled = Mage::helper('qquoteadv')->isEnabled() == 1;
        $isAllowedToQuote = $this->getProduct()->getData('allowed_to_quotemode') == 1;
        $isHideQuoteButton = Mage::helper('qquoteadv')->hideQuoteButton($this->getProduct());
        $isDetailPageActivated = Mage::getStoreConfig('qquoteadv_quote_frontend/catalog/layout_update_detailpage_activated') == 1;
