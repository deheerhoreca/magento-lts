var OnecolumnCheckout = {
    ...
    _handleScroll: function(scroll) {
        ...
        if (sidebar && sidebar.getStyle('display') !== 'none') {
            var scrollingSpaceNeeded = step.viewportOffset().top - FC.Sticky.prototype.config.offset_top,
            ...
            if (scrollingSpaceNeeded > availableScrollingSpace) {
                if (step.viewportOffset().top === sidebar.viewportOffset().top) { // first step
                    ...
                    availableScrollingSpace =
                        (document.body.getHeight() - $('scrolling-spacer').getHeight()) -
    ...
    addObservers: function() {
        ...
        var guestCheckbox = $('billing\:guest_account'),
            registerCheckbox = $('billing\:register_account');
        if (guestCheckbox) {
            ...
            if (registerCheckbox) {
                registerCheckbox.observe('click', function(e) {
...
var OrderSummary = {
    cloneOrderTotals: function() {
        ...
        // Amasty Orderattr integration
        if (div.down('.fields-off')) {
            div.down('.fields-off').remove();
        }
    },
